...
M1 已按[详细计划](superpowers/plans/2026-09-06-m1-first-voyage.md)完成并通过验收。最新接续入口见[交接文档](../HANDOFF.md)：
...
M2 已按 [M2 计划](superpowers/plans/2026-09-06-m2-conflict-and-growth.md)完成。战斗规则、存档 v6、结算恢复、整备界面、声明式剧情接口及完整章节均已接入；准确证据、预览地址和后续工作见[交接文档](../HANDOFF.md)与[M2 验证记录](superpowers/verification/2026-09-06-m2-conflict-and-growth.md)。

- 决斗采用直接攻击/防御选择和十回合平局；海战使用旗舰与四档距离，实际结算炮弹、木材、耐久和船员。
- 装备影响战斗；船厂可按资金部分修理；经验与等级可保存并显示。进行中战斗可存读档，系统弹窗隔离底层输入。
- 本章流程为休达多明戈身份事件、里斯本卡恩交锋后的收束、卡特琳娜追击及阿兰寻妹，结束于伊斯坦布尔旅馆回报。
- 胜利和主动撤退推进追击；战败回里斯本恢复，在码头直接重试。多明戈离队会完成船长交接，必要时加入原创代理船长并保留四船舰队。
- 采用可达的章节里程碑代替原作 2000/8000 名声门槛。现有 14 处发现物合计 1380 冒险名声，后续门槛仍须按实际可获得奖励核算。完整六角海战、多船指令、昼夜战斗上限及缴获留待后续扩展。

最终修补 `2dc2987` 已通过全量 86 组/662 项单元检查、受影响的 21 项浏览器场景及定向复审；此前 80/80 全量浏览器回归保留为完整基线，两个首次开局长旅程未在末次范围回归重复。M2 按计划保留在 `codex/m2-conflict-and-growth`，8082 可试玩，尚未合并推送。

## M3 约翰后续主线（待实施）

从 M2 的阿兰寻妹报告继续，规划马萨瓦与圣者之杖、卡特琳娜误会的后续解决、日本、南美和最终战。实施前先核对原作前置、地点、连续海上天数与胜败出口，再定义当前内容可达到的章节门槛。继续保留旧档、拒绝重试、一次性奖励、伙伴/舰船引用和真实航行回归。
