...
**Status**: Reference · **Date**: 2026-06-13
...
> **Update (2026-06-13)**: The "Trade Goods" target below has partly landed —
> `src/data/goodsData.ts` and `src/data/marketGoodsData.ts` now exist (commit
> `9593a11`), so trade goods are no longer a missing dataset. Treat that section
> as a schema checklist to *reconcile against* the shipped files, not a from-zero
> build. Naming note: the shipped files are `goodsData` / `marketGoodsData`, not
> the placeholder `tradeGoodData` once proposed here.
